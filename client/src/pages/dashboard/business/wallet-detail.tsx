...
import { ArrowLeft, TrendingUp, TrendingDown, Banknote, ArrowDownCircle, ArrowUpCircle } from "lucide-react";
...
export default function WalletDetail() {
  ...
  if (useCustom && customFrom && customTo) {
    ...
    queryParams.set("days", String(selectedDays ?? 30));
  }
  const currency = info?.currency;
  if (currency) queryParams.set("currency", currency);

  const { data, isLoading } = useQuery<any>({
    queryKey: [`/api/business/wallet-stats/${countryKey}`, queryParams.toString()],
    queryFn: async () => {
      const res = await fetch(`/api/business/wallet-stats/${countryKey}?${queryParams}`);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <Link href="/dashboard/business">
          <Button variant="ghost" size="icon" data-testid="button-wallet-back">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Détails du wallet</p>
        </div>
      </div>
