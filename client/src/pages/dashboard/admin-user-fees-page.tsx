...
import {
  ArrowLeft, Percent, Save, ChevronDown, ChevronRight,
...
export default function AdminUserFeesPage() {
  ...
  const { data: userFees = [], isLoading: userFeesLoading } = useQuery<FeeConfig[]>({
    queryKey: [`/api/admin/users/${userId}/fee-configs`],
    enabled: !!userId,
  });
  ...
  const upsertMutation = useMutation({
    mutationFn: async (payload: { provider: string; country: string; operator: string; incomingFeePercentage?: number; outgoingFeePercentage?: number }) => {
      return apiRequest("PUT", `/api/admin/users/${userId}/fee-configs/${payload.provider}/${payload.country}/${payload.operator}`, {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/admin/users/${userId}/fee-configs`] });
    },
    onError: (err: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (payload: { provider: string; country: string; operator: string }) => {
      return apiRequest("DELETE", `/api/admin/users/${userId}/fee-configs/${payload.provider}/${payload.country}/${payload.operator}`);
    },
    onSuccess: () => {
  ...
  const totalCustom = userFees.length;
    ...
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        <Button
          ...
          data-testid="button-back-user-fees"
        >
        ...
        <div className="flex-1 min-w-0">
          {userLoading ? (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <h1 className="text-xl font-bold text-foreground truncate" data-testid="title-user-fees">
                Frais personnalises — {userName}
              </h1>
              {totalCustom > 0 && (
                <Badge className="bg-amber-500 text-white text-xs">{totalCustom} perso.</Badge>
              )}
            </div>
          )}
          <p className="text-sm text-muted-foreground mt-0.5">
            Configurez des frais specifiques pour cet utilisateur. Sans frais personnalises, les frais globaux business s'appliquent.
          </p>
        </div>
      </div>
      ...
      {userFeesLoading ? (
        ...
        <Tabs value={activeProvider} onValueChange={(v) => { setActiveProvider(v); setExpandedCountry(null); }}>
                  ...
                  return (
                    <div key={`${provider.id}-${country.code}`} className="border rounded-md">
                      <button
                        ...
                        className="w-full flex items-center justify-between gap-2 p-3 hover:bg-muted/50 transition-colors rounded-md"
