...
app.post('/api/webhook/bkapay-payout', express.raw({ type: 'application/json' }), (req, res) => {
  const rawBody = req.body.toString('utf8');
  const payload = JSON.parse(rawBody);
  const signature = req.headers['x-bkapay-signature'];
  const secret = process.env.BKAPAY_PAYOUT_CALLBACK_SECRET;
  ...
  if (event === 'payout.completed') {
    updatePayoutStatus(reference, 'paid');   // Virement reussi
  } else if (event === 'payout.failed') {
    notifyFailure(transactionId, reference); // Virement echoue
  } else if (event === 'payout.pending') {
    // Virement en cours de traitement (optionnel)
  }
        ...
        <CardContent className="space-y-4 text-sm">
          ...
          <Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950 dark:border-blue-800">
            <AlertDescription className="text-blue-800 dark:text-blue-200 text-xs space-y-1">
              <p>
                Chaque webhook payout est signe avec votre <strong>secret de signature payout</strong> (<code className="font-mono bg-blue-100 dark:bg-blue-900 px-1 rounded">BKAPAY_PAYOUT_CALLBACK_SECRET</code>) visible dans la section <strong>API Payout</strong> de votre tableau de bord.
                Verifiez toujours la signature dans le header <code className="font-mono bg-blue-100 dark:bg-blue-900 px-1 rounded">X-BKApay-Signature</code> avant de traiter le webhook. Ce secret est distinct du secret webhook payin (<code className="font-mono bg-blue-100 dark:bg-blue-900 px-1 rounded">BKAPAY_CALLBACK_SECRET</code>) visible dans la section <strong>Cles API</strong>.
              </p>
              <p>
