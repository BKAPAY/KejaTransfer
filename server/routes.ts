...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Statistiques détaillées d'un wallet entreprise (vue admin)
  app.get("/api/admin/business/wallet-stats/:userId/:country", requireAdmin, async (req: Request, res: Response) => {
    try {
      const pool = new pg.Pool({ connectionString: process.env.DATABASE_URL });
...
- Retourne UNIQUEMENT le message ameliore, sans commentaire ni explication`
  ...
  app.delete("/api/admin/user/:userId/salary", requireAdmin, async (req: Request, res: Response) => {
    try {
      const { userId } = req.params;
      await storage.deleteSalaryAccountCompletely(userId);
      await storage.updateUserSalaryFlag(userId, false);
      return res.json({ success: true });
    } catch (error: any) {
      return res.status(500).json({ success: false, error: "Erreur serveur" });
  ...
  // Sample products for each category type
  ...
  app.post("/api/shop", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // Créer les produits exemples par défaut et marquer comme seedé
      const rawPool = new pg.Pool({ connectionString: process.env.DATABASE_URL });
      for (let i = 0; i < DEFAULT_CATEGORIES.length; i++) {
        const catName = DEFAULT_CATEGORIES[i].name;
        const key = Object.keys(SAMPLE_MAP).find(k => catName.toLowerCase().includes(k.toLowerCase()));
        const samples = key ? SAMPLE_MAP[key] : FALLBACK;
        // On a besoin de l'ID de la catégorie créée
        const catRes = await rawPool.query(`SELECT id FROM shop_categories WHERE shop_id = $1 AND name = $2`, [shop.id, catName]);
        ...
        if (catId) {
          for (const s of samples) {
            await rawPool.query(
              ...
              [shop.id, catId, s.name, s.desc, s.price, s.img]
            );
          }
        }
      }
      // Marquer la boutique comme seedée
      await rawPool.query(`UPDATE shops SET seeded_at = NOW() WHERE id = $1`, [shop.id]);
      await rawPool.end();
  ...
  app.post("/api/shop/seed-defaults", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      // IMPORTANT: Ne jamais re-seeder une boutique déjà seedée
      if ((shop as any).seededAt) {
        console.log(`[Seed] Boutique ${shop.id} déjà seedée le ${(shop as any).seededAt}, ignoré`);
        return res.json({ success: true, created: 0, message: "Produits exemples déjà créés" });
      }
      ...
      // Marquer la boutique comme seedée (même si 0 produits créés)
      await rawPool.query(`UPDATE shops SET seeded_at = NOW() WHERE id = $1`, [shop.id]);

      await rawPool.end();
      console.log(`[Seed] Créé ${createdCount} produits pour boutique ${shop.id} - boutique marquée comme seedée`);
